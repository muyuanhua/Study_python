...
from util.runmenthod import RunMethod
...
class DependentData:
    ...
    def run_dependent(self):
        ...
        method=self.data.get_request_url(row_num)
        url=self.data.get_request_url(row_num)
        res=self.method.run_main(method,url,request_data,header,params=request_data)
        return res

    #获取依赖字段的响应数据：通过执行依赖测试case来获取响应数据，响应中的某个字段作为依赖的key的value
    def get_value_for_key(self,row):
        #获取依赖的返回数据key
        depend_data=self.data.get_depend_key(row)
        #执行依赖case的返回结果
        respones_data=self.run_dependent()
        #print(depend_data)
        #print(response_data)
        return [match.value for match in parser(depend_data).find(respones_data)][0]
